...
void render_header(std::shared_ptr<JsonValue> root, std::ostream& out) {
    ...
    std::map<int, std::string> platform_keys_by_id;
    ...
    for (const auto& p : platforms) {
        ...
        if (platform_ids.find(key) != platform_ids.end()) {
            throw std::runtime_error("Duplicate platform key: " + key);
        }
        const auto existing_id = platform_keys_by_id.find(pid);
        if (existing_id != platform_keys_by_id.end()) {
            throw std::runtime_error("Duplicate platform id " + std::to_string(pid) +
                                     " for " + existing_id->second + " and " + key);
        }
        platform_ids[key] = pid;
        platform_keys_by_id[pid] = key;
